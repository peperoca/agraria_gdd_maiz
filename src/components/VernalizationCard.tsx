...
export function VernalizationCard({ gddData, target }: VernalizationCardProps) {
  ...
  return (
    <div className="agraria-card">
      <div className="sec-label">{t('vernalization.title')}</div>

      <div className="agraria-info-row grid grid-cols-3 gap-2 mb-3">
        <div>
          <div className="text-[11px] opacity-75" style={{ color: 'var(--it)' }}>{t('vernalization.vdHours')}</div>
          <div className="text-base font-semibold" style={{ color: 'var(--it)' }}>
            {Math.round(latest.cumulativeVd)}
          </div>
        </div>
        <div>
          <div className="text-[11px] opacity-75" style={{ color: 'var(--it)' }}>{t('vernalization.target')}</div>
          <div className="text-base font-semibold" style={{ color: 'var(--it)' }}>
            {target}
          </div>
        </div>
        <div>
          <div className="text-[11px] opacity-75" style={{ color: 'var(--it)' }}>{t('vernalization.status')}</div>
      ...
      <div className="flex justify-between mt-1">
        <span className="text-[10px]" style={{ color: 'var(--tx3)' }}>
          {t('vernalization.progressLabel', { current: Math.round(latest.cumulativeVd), target })}
        </span>
        <span className="text-[10px]" style={{ color: 'var(--tx3)' }}>
          {complete ? t('vernalization.complete') : t('vernalization.remaining', { amount: target - Math.round(latest.cumulativeVd) })}
        </span>
      </div>
      ...
      {latest.dailyVd > 0 && (
        <p className="text-[10px] mt-2" style={{ color: 'var(--tx3)' }}>
          {t('vernalization.todayContribution', { hours: latest.dailyVd })}
        </p>
      )}

      <p className="text-[10px] mt-1" style={{ color: 'var(--tx3)' }}>
        {t('vernalization.description')}
      </p>
    </div>
  );
}
